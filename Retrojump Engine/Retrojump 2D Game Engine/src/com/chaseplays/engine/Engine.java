...
public class Engine extends Canvas {
	...
	public void render (Game e) {
		...
		e.render(); // renders everything in the game
	...
	public void run (Game e) {
		...
		long lastUpdated = System.currentTimeMillis(); // sets the time last updated
		
		long now = System.currentTimeMillis(); // sets the time now
		...
		while (running) { // loops as long as the game is running
			...
			while (now - lastUpdated >= 1) { // loops as long as it has been at least one millisecond since the last update
				
				lastUpdated += 1; // adds one millisecond to last updated
	...
	public void checkSounds () {
		...
		if (sounds.size() > 0) {
			...
			for (int x = 0; x < sounds.size(); x++) {
				...
				if (sounds.get(x).clip.isRunning()) sounds.remove(x);
