...
public class Renderer {
	...
	public void render (int x, int y, Sprite sprite) {
		...
		for (int spriteY = 0; spriteY < sprite.height; spriteY++) {
			...
			for (int spriteX = 0; spriteX < sprite.width; spriteX++) {
				...
				double alpha = (double) (sprite.alphas[spriteX + spriteY * sprite.width]) / (double) 255;
				
				if (!(screenPixelX < 0 || screenPixelX >= screen.width || screenPixelY < 0 || screenPixelY >= screen.height)) {
			...
			//		System.out.println("HI " + screenPixelX + " " + screen.width);
	...
	//		System.out.println("HI " + screenPixelY);
			
		}
		
	//	System.out.println(yLoops);
